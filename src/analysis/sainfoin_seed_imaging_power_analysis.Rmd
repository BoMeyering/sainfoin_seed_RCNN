...
# p_seed_matrix <- matrix(nrow = nrow(combined_data), ncol = nrow(combined_data))
# es_seed_matrix <- matrix(nrow = nrow(combined_data), ncol = nrow(combined_data))
# sum_count <- c()
# factor_g <- c()
# 
# for(i in 1:nrow(combined_data)){
#         for(j in 1:nrow(combined_data)){
#                 es_prop = ES.h(combined_data[i,]$p_seed, combined_data[j,]$p_seed)
#                 pair_power = pwr.2p2n.test(h=es_prop,
#                               n1=combined_data[i,]$total_seeds, 
#                               n2=combined_data[i,]$total_seeds,
#                               alternative="two.sided")
#                 factor_g <- c(factor_g, combined_data[i,]$sample_mass_g)
#                 sum_count <- c(sum_count, combined_data[i,]$total_seeds*combined_data[j,]$total_seeds)
#                 es_seed_matrix[i,j] = es_prop
#                 p_seed_matrix[i,j] = pair_power$power
#                 
#         }
# }
# 
# es_vec <- abs(as.vector(es_seed_matrix))
# pwr_vec <- as.vector(p_seed_matrix)
# 
# pes_df <- data.frame("es"=es_vec, "sum_count"=sum_count, "pwr"=pwr_vec, 'g'=factor_g)
# 
# ggplot(pes_df, aes(x = es_vec, y=pwr, color=sum_count))+
#         geom_point(alpha=.5)+
#         scale_color_viridis_c(option='D', trans="log")
...
```{r deprecated?}
variety = rep(c("AAC Mountainview", "Delaney", "Eski", "Rocky Mountain Remont", "Shoshone"), each = 2000)
method = rep(c("HLDP7x35s", "BTX3"), times=10000)
sample_mass = rep(seq(from=0.01, to = 5.0, length=1000), each=2, times = 5)
X_mat <- data.frame(variety=variety, method=method, legume_fruit_mass_g=sample_mass)

X_mat$pred_count <- as.integer(predict(seed_model, X_mat))
X_mat$pred_count <- factor(X_mat$pred_count, ordered = TRUE)
X_mat <- X_mat %>% 
        group_by(variety, pred_count, method) %>% 
        summarize(legume_fruit_mass_g = mean(legume_fruit_mass_g)) %>% 
        filter(pred_count > 0) %>% 
        mutate(n_char = as.character(pred_count))

final_df <- X_mat %>% 
        left_join(p_sizes, by = 'n_char') %>% 
        mutate(spread_char = as.character(prop_spread)) %>% 
        group_by(variety, spread_char, method) %>% 
        summarize(legume_fruit_mass_g = mean(legume_fruit_mass_g),
                  prop_spread = mean(prop_spread),
                  pred_count = mean(as.numeric(pred_count)))
```


```{r broken plot}
ggplot(final_df, aes(x = legume_fruit_mass_g, y = prop_spread, color = variety))+
        geom_line()+
        scale_color_viridis_d(option='D')+
